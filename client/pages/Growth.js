...
import { ChevronRight, PiggyBank, Wallet, LineChart as LineChartIcon, Sparkles, Target, ArrowRight } from "lucide-react";
    ...
    return (<div className="p-8 md:p-12 max-w-7xl mx-auto space-y-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <div className="flex items-center gap-2 bg-white/5 border border-white/10 p-1.5 rounded-2xl shrink-0">
          {["1m", "3m", "6m", "10m", "1y", "ALL"].map(t => (<button key={t} onClick={() => setTimeframe(t)} className={cn("px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", timeframe === t ? "bg-primary text-background shadow-lg" : "text-muted-foreground hover:text-white")}>
              {t}
            </button>))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="flex-1 w-full mt-6">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={growthData}>
                ...
                <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: "rgba(255,255,255,0.2)", fontSize: 11, fontWeight: 700 }} dy={20}/>
            ...
            boxShadow: '0 20px 40px rgba(0,0,0,0.5)'
                ...
                <Area type="monotone" dataKey="target" stroke="rgba(255,255,255,0.1)" strokeWidth={2} strokeDasharray="10 10" fill="transparent"/>
                <Area type="monotone" dataKey="balance" stroke="#2DED9C" strokeWidth={4} fillOpacity={1} fill="url(#colorBalance)" dot={{ r: 4, fill: "#2DED9C", strokeWidth: 2, stroke: "#12110F" }} activeDot={{ r: 8, fill: "#2DED9C", strokeWidth: 0, shadow: '0 0 20px rgba(45,237,156,0.5)' }}/>
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ...
        ].map((stat, i) => (<div key={i} className="space-y-1">
        ...
        <div className="lg:col-span-1 space-y-6 flex flex-col h-full">
           <div className="bg-[#12110F] border border-white/5 rounded-[2.5rem] p-8 space-y-8 flex-1 flex flex-col">
            ...
            return (<div key={goal.id} className="space-y-3 group cursor-pointer">
                     <div className="flex items-center justify-between">
                     ...
                     <div className="space-y-1.5">
                       ...
                       <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                         <div className={cn("h-full rounded-full transition-all duration-[1500ms]", progress > 0 ? "bg-primary" : "bg-white/10")} style={{ width: `${progress}%` }}/>
                       </div>
                     </div>
                   </div>);
        })}
