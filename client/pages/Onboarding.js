...
import { ChevronLeft, ArrowRight, Check, Shield, Zap, Brain, Target, Lock, Sparkles } from "lucide-react";
...
export default function Onboarding() {
    ...
    const SlideContainer = ({ title, subtitle, children, nextLabel = "Next" }) => (<div className="flex-1 flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="mt-auto pt-10 flex gap-4">
        {currentStep > 0 && (<Button variant="outline" size="lg" onClick={back} className="rounded-2xl h-16 w-16 border-white/10 bg-white/5 hover:bg-white/10">
            <ChevronLeft className="w-6 h-6"/>
          </Button>)}
        <Button size="lg" onClick={currentStep === totalSteps - 1 ? finish : next} className="flex-1 rounded-2xl h-16 bg-primary text-background font-black uppercase tracking-widest text-sm hover:scale-[1.02] active:scale-[0.98] transition-all shadow-[0_0_30px_rgba(45,237,156,0.3)]">
      ...
      <div className="w-full max-w-[420px] h-[840px] bg-[#12110F] border border-white/5 rounded-[3.5rem] shadow-2xl relative z-10 flex flex-col overflow-hidden p-10">
        ...
        <div className="pt-10 mb-8 flex justify-between items-center">
          <div className="flex gap-1.5 flex-1">
            {Array.from({ length: totalSteps }).map((_, i) => (<div key={i} className={cn("h-1 rounded-full transition-all duration-500", i <= currentStep ? "bg-primary w-8" : "bg-white/10 w-2")}/>))}
          </div>
        ...
        <div className="flex-1 flex flex-col">
          {currentStep === 0 && (<div className="flex-1 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in duration-1000">
              ...
              <Button size="lg" onClick={next} className="w-full rounded-2xl h-16 bg-primary text-background font-black uppercase tracking-widest text-sm hover:scale-[1.02] active:scale-[0.98] transition-all shadow-[0_0_30px_rgba(45,237,156,0.3)] mt-auto">
                Establish Uplink
                <ArrowRight className="ml-2 w-5 h-5"/>
              </Button>
            </div>)}

          {currentStep === 1 && (<SlideContainer title="I'm Nova." subtitle="Your Advanced Financial AI Consultant.">
          ...
          {currentStep === 2 && (<SlideContainer title="Pure Integrity." subtitle="Why we connect directly to your institutions.">
            ...
            ].map((item, i) => (<div key={i} className="flex items-start gap-4 p-5 bg-white/5 rounded-[2rem] border border-white/5">
          ...
          {currentStep === 3 && (<SlideContainer title="The Mirror." subtitle="Correlating your mood with your capital.">
              <div className="space-y-6 pt-4">
          ...
          {currentStep === 4 && (<SlideContainer title="Intentions." subtitle="What are we building together?">
              <div className="space-y-3 pt-4">
                {['Debt Elimination', 'Wealth Accrual', 'Peace of Mind', 'Capital Growth'].map(goal => (<button key={goal} onClick={() => toggleGoal(goal)} className={cn("w-full p-6 rounded-2xl border text-left transition-all flex justify-between items-center group", goals.includes(goal) ? "bg-primary/10 border-primary text-primary" : "bg-white/5 border-white/10 text-muted-foreground hover:border-white/20")}>
          ...
          {currentStep === 5 && (<SlideContainer title="Your Rhythm." subtitle="Set your monthly baseline expenditure.">
          ...
          {currentStep === 6 && (<SlideContainer title="The Vault." subtitle="Permanent, encrypted storage for your records.">
              <div className="space-y-6 pt-4">
          ...
          {currentStep === 7 && (<SlideContainer title="Evolving." subtitle="Choose my interaction protocol.">
              <div className="space-y-4 pt-4">
                {[
                ...
                { id: 'Empathetic', desc: 'Supportive guidance focused on well-being.', color: 'text-blue-400' }
            ].map(tone => (<button key={tone.id} onClick={() => setTone(tone.id)} className={cn("w-full p-6 rounded-2xl border text-left transition-all group relative overflow-hidden", novaTone === tone.id ? "bg-white/10 border-primary" : "bg-white/5 border-white/10 hover:border-white/20")}>
                    ...
                    {novaTone === tone.id && (<div className="absolute right-4 top-1/2 -translate-y-1/2">
          ...
          {currentStep === 8 && (<div className="flex-1 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in duration-1000">
              ...
              <Button size="lg" onClick={finish} className="w-full rounded-2xl h-16 bg-primary text-background font-black uppercase tracking-widest text-sm hover:scale-[1.02] active:scale-[0.98] transition-all shadow-[0_0_30px_rgba(45,237,156,0.3)] mt-auto">
