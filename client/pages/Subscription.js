...
import { Check, ChevronRight, Sparkles, ShieldCheck, Crown, Star, ArrowRight } from "lucide-react";
...
export default function SubscriptionPage() {
    ...
    return (<div className="p-8 md:p-12 max-w-7xl mx-auto space-y-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <div className="bg-white/5 border border-white/10 p-2 rounded-2xl flex items-center shrink-0">
          <button onClick={() => setIsAnnual(false)} className={cn("px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all", !isAnnual ? "bg-white/10 text-white shadow-lg" : "text-muted-foreground hover:text-white")}>
            Monthly
          </button>
          <button onClick={() => setIsAnnual(true)} className={cn("px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all relative", isAnnual ? "bg-white/10 text-white shadow-lg" : "text-muted-foreground hover:text-white")}>
            Annual
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-6">
        {plans.map((plan, i) => (<div key={i} className={cn("bg-[#12110F] border border-white/5 rounded-[2.5rem] p-10 flex flex-col h-full relative transition-all group hover:bg-[#151412] hover:border-white/10", plan.popular ? "ring-2 ring-primary/30 shadow-[0_0_50px_rgba(45,237,156,0.1)] scale-105 z-10" : "scale-100")}>
            {plan.popular && (<div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-primary text-background text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-[0.2em] shadow-[0_0_20px_rgba(45,237,156,0.5)]">
                Most Popular
              </div>)}
            ...
            <div className="space-y-4 mb-12 flex-1 pt-6 border-t border-white/5">
              {plan.features.map((feature, fi) => (<div key={fi} className="flex items-start gap-3 group/feature">
            ...
            <button className={cn("w-full py-4 rounded-2xl font-black text-sm transition-all flex items-center justify-center gap-2 uppercase tracking-widest", plan.popular
