...
import { ArrowDownRight, ArrowUpRight, ChevronRight, Play, Flame, Zap, AlertCircle, Loader2, Sparkles } from "lucide-react";
...
const StatCard = ({ label, value, trend, trendValue, icon: Icon, colorClass }) => (<div className="bg-[#0A0907] border border-white/[0.03] rounded-3xl p-6 relative overflow-hidden group hover:bg-[#11100D] transition-all hover:border-white/10 shadow-2xl">
...
const TriggerCard = ({ title, subtitle, icon: Icon, stats, chartColor, aiTip, colorClass }) => (<div className="bg-[#0A0907] border border-white/[0.03] rounded-[3rem] p-8 flex flex-col h-full group hover:border-white/10 transition-all hover:bg-[#0E0D0B] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] relative overflow-hidden">
    {/* Scanline Overlay */}
    ...
    <div className="grid grid-cols-3 gap-6 mb-8 pt-6 border-t border-white/5">
      {stats.map((stat, i) => (<div key={i}>
          ...
          <div className="text-lg font-bold text-white flex items-center gap-1">
            {stat.value}
            {stat.trend && (<span className={cn("text-[10px] font-bold", stat.trend === "up" ? "text-red-400" : "text-primary")}>
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={chartData}>
          <Bar dataKey="value" radius={[6, 6, 6, 6]}>
            {chartData.map((entry, index) => (<Cell key={`cell-${index}`} fill={index === 3 ? chartColor : "rgba(255,255,255,0.03)"} className="transition-all duration-500 hover:opacity-80"/>))}
          </Bar>
...
export default function Index() {
    ...
    return (<div className="p-4 md:p-8 max-w-7xl mx-auto space-y-10">
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <button onClick={runAnalysis} disabled={analyzing} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 px-5 py-3 rounded-full text-[10px] font-black transition-all group shadow-lg uppercase tracking-widest disabled:opacity-50">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Balance" value={`$${stats?.totalBalance?.toLocaleString()}`} trend="up" trendValue="8%" colorClass="bg-primary"/>
        <StatCard label="Target Recovery" value="$10,000" trend="down" trendValue="Critical" colorClass="bg-red-500" icon={AlertCircle}/>
        <StatCard label="Monthly Baseline" value={`$${stats?.baselineSpend || 2500}`} trend={stats?.monthlyExpenses > (stats?.baselineSpend || 2500) ? "up" : "down"} trendValue={`${Math.abs(Math.round(((stats?.monthlyExpenses || 0) / (stats?.baselineSpend || 1)) * 100 - 100))}%`} colorClass="bg-blue-400"/>
        <StatCard label="Mindful Streak" value="14d" trend="up" trendValue="2d" colorClass="bg-primary" icon={Flame}/>
      </div>
      ...
      <div className="bg-red-500/5 border border-red-500/20 rounded-[2.5rem] p-8 relative overflow-hidden group">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
          ...
          <div className="w-full md:w-64 space-y-3">
            ...
            <button onClick={executeDemand} className="w-full bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {stats?.triggers?.map((trigger) => (<TriggerCard key={trigger.id} title={trigger.name} subtitle={trigger.description || "Identified behavioral trajectory"} icon={Zap} colorClass={trigger.status === 'Critical' ? 'text-red-400' : 'text-primary'} chartColor={trigger.status === 'Critical' ? '#ef4444' : '#2DED9C'} stats={[
        ...
        {(!stats?.triggers || stats?.triggers?.length === 0) && (<div className="col-span-2 p-12 bg-[#12110F] border border-dashed border-white/10 rounded-[2.5rem] text-center">
