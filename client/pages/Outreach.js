...
import { Send, Upload, BarChart3, ChevronRight, Loader2, Sparkles } from "lucide-react";
import { cn } from "@/lib/utils";
import { Badge } from "@/components/ui/badge";
import { useToast } from "@/components/ui/use-toast";
const CampaignCard = ({ title, status, recipients, openRate, clickRate }) => (<div className="bg-[#0A0907] border border-white/[0.03] rounded-3xl p-6 transition-all hover:bg-[#11100D] hover:border-white/10 group">
...
export default function Outreach() {
    ...
    return (<div className="p-4 md:p-8 max-w-7xl mx-auto space-y-10 pb-20">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="bg-[#0A0907] border border-white/[0.03] rounded-[2.5rem] p-8 space-y-8 shadow-2xl relative overflow-hidden group">
            ...
            <div className="space-y-6 relative z-10">
              <div className="space-y-2">
                ...
                <input type="text" placeholder="e.g., Exclusive Opportunity: High-Fidelity Solutions" value={subject} onChange={(e) => setSubject(e.target.value)} className="w-full bg-white/[0.02] border border-white/5 rounded-2xl py-5 px-6 focus:outline-none focus:border-primary/40 transition-all font-bold text-white placeholder:text-muted-foreground/30"/>
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black text-primary uppercase tracking-[0.2em] ml-2">Broadcast Message</label>
                <textarea rows={8} placeholder="Draft your high-sophistication message here..." value={message} onChange={(e) => setMessage(e.target.value)} className="w-full bg-white/[0.02] border border-white/5 rounded-3xl py-6 px-6 focus:outline-none focus:border-primary/40 transition-all font-medium text-white placeholder:text-muted-foreground/30 resize-none leading-relaxed"/>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-8 border-t border-white/5">
              ...
              <button onClick={deployBroadcast} disabled={sending || !leadsCount} className="flex-[2] bg-primary text-background rounded-2xl py-4 flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] hover:scale-[1.02] active:scale-[0.98] transition-all shadow-[0_0_40px_rgba(45,237,156,0.2)] disabled:opacity-50">
        ...
        <div className="space-y-8">
        ...
        ].map((stat, i) => (<div key={i} className="flex justify-between items-center py-3 border-b border-white/5 last:border-0">
          ...
          <div className="space-y-4">
            ...
            <CampaignCard title="Wealth Transfer Early Access" status="Active" recipients="850" openRate="42" clickRate="12"/>
            <CampaignCard title="Pulse Beta Launch" status="Completed" recipients="1,200" openRate="38" clickRate="9"/>
          </div>
        </div>
      </div>
    </div>);
