...
import { Send, Sparkles, AlertCircle, CheckCircle2, Moon, Zap, Smartphone, Info } from "lucide-react";
...
const StressIndex = ({ value }) => {
    const circumference = 2 * Math.PI * 40;
    const offset = circumference - (value / 100) * circumference;
    return (<div className="relative w-32 h-32 flex items-center justify-center mx-auto mb-4">
      <svg className="w-full h-full -rotate-90">
        <circle cx="64" cy="64" r="40" stroke="currentColor" strokeWidth="8" fill="transparent" className="text-white/5"/>
        <circle cx="64" cy="64" r="40" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" className="text-orange-400 shadow-[0_0_15px_rgba(251,146,60,0.5)] transition-all duration-1000"/>
      </svg>
...
const MetricRow = ({ label, value, color }) => (<div className="flex items-center justify-between mb-2">
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data}>
          <Bar dataKey="v" radius={[2, 2, 0, 0]}>
            {data.map((_, i) => (<Cell key={i} fill={i === data.length - 1 ? "#2DED9C" : "rgba(45,237,156,0.2)"}/>))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </div>);
...
export default function NovaChat() {
    ...
    return (<div className="flex h-[calc(100vh-64px)] md:h-screen overflow-hidden">
      {/* Main Chat Area */}
      <div className="flex-1 flex flex-col min-w-0 bg-background relative">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-hide">
          {chatMessages.map((msg, i) => (<div key={i} className={cn("flex flex-col", msg.role === "user" ? "items-end" : "items-start")}>
              {/* ... same message rendering logic ... */}
              {msg.type === "alert" ? (<div className="w-full max-w-2xl bg-orange-500/10 border border-orange-500/20 rounded-2xl p-4 flex items-center justify-between group">
                  <div className="flex items-center gap-3">
                ...
                </div>) : msg.type === "insight" ? (<div className="max-w-2xl bg-[#1A1816] border border-white/5 rounded-3xl p-6 space-y-6 shadow-2xl">
                  ...
                  <div className="grid grid-cols-3 gap-6 pt-4 border-t border-white/5">
                    {msg.stats?.map((stat, si) => (<div key={si}>
                        <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider mb-1">{stat.label}</div>
                        <div className={cn("text-xl font-bold", stat.color)}>{stat.value}</div>
                      </div>))}
                  </div>
                </div>) : (<div className={cn("max-w-[80%] rounded-3xl p-4 text-sm leading-relaxed", msg.role === "user"
          ...
          {isTyping && (<div className="items-start flex flex-col">
        ...
        <div className="p-6 shrink-0 bg-background/80 backdrop-blur-md">
          <div className="max-w-4xl mx-auto flex flex-wrap gap-2 mb-4">
             {["Show monthly trigger cost", "Set pause reminder", "What are my top triggers?", "I want to break this habit"].map(chip => (<button key={chip} onClick={() => { setInput(chip); }} className="px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-[11px] font-medium hover:bg-white/10 transition-colors">
                 {chip}
               </button>))}
          </div>
          <div className="max-w-4xl mx-auto relative group">
            <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Message Nova..." className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 pr-14 focus:outline-none focus:border-primary/50 transition-colors text-sm"/>
            <button onClick={handleSend} className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-xl bg-primary flex items-center justify-center hover:scale-105 active:scale-95 transition-all">
      ...
      <div className="hidden xl:flex flex-col w-80 shrink-0 bg-[#0F0E0D] border-l border-white/5 p-6 overflow-y-auto">
        ...
        ].map((t, i) => (<div key={i} className="flex items-center justify-between group cursor-pointer hover:translate-x-1 transition-transform">
                <div className="flex items-center gap-3">
