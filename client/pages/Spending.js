import { ChevronRight, Search, Filter, Download, ShoppingBag, Utensils, Smartphone, Car, Coffee, MoreVertical, Zap, Flame, PieChart as PieChartIcon } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <div className="flex items-center gap-4">
          <div className="relative group">
            ...
            <input placeholder="Search transactions..." className="bg-white/5 border border-white/5 pl-12 pr-6 py-3 rounded-full text-xs font-bold text-white focus:outline-none focus:border-primary/30 transition-all w-64"/>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-[#12110F] border border-white/5 rounded-[2.5rem] p-10 space-y-8 flex flex-col items-center justify-center">
          ...
          <div className="h-64 w-full relative">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={categoryData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={8} dataKey="value">
          ...
          <div className="w-full space-y-4 pt-6 border-t border-white/5">
            {categoryData.map((cat, i) => (<div key={i} className="flex items-center justify-between group cursor-pointer hover:bg-white/5 p-2 rounded-xl transition-all">
        ...
        <div className="lg:col-span-2 bg-[#12110F] border border-white/5 rounded-[2.5rem] p-10 space-y-10">
          ...
          <div className="space-y-4">
            {transactions.map((tx) => (<div key={tx.id} className="group flex items-center justify-between p-5 rounded-3xl hover:bg-white/[0.03] border border-transparent hover:border-white/5 transition-all">
                ...
                <div className="flex flex-col items-end gap-3 shrink-0 ml-4">
                   ...
                   <div className="flex items-center gap-2">
                     {tx.trigger !== "None" && (<Badge variant="outline" className="bg-red-500/10 text-red-400 border-red-500/20 px-2.5 py-0 text-[9px] font-black flex gap-1 uppercase tracking-widest">
                         <Zap className="w-3 h-3 fill-red-400"/>
                         {tx.trigger}
                       </Badge>)}
                     <div className={cn("text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest", tx.status === "Safe" ? "bg-primary/10 text-primary border border-primary/20" :
