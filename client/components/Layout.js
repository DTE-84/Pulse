...
import { LayoutDashboard, MessageSquare, CreditCard, TrendingUp, Zap, Settings, User } from "lucide-react";
...
const Sidebar = () => {
    ...
    return (<aside className="hidden lg:flex flex-col w-72 h-screen bg-[#0A0907] border-r border-white/5 sticky top-0 overflow-hidden">
      <div className="p-8">
        ...
        <nav className="space-y-2">
          {menuItems.map((item) => {
            ...
            return (<Link key={item.label} to={item.href} className={cn("flex items-center gap-4 px-5 py-4 rounded-2xl text-sm font-bold transition-all group relative overflow-hidden", isActive
                ...
                {item.label}
                {isActive && (<div className="absolute right-4 w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_10px_rgba(45,237,156,1)]"/>)}
              </Link>);
        })}
        </nav>
      </div>
      ...
      <div className="mt-auto p-8 border-t border-white/5 bg-black/20">
        <div onClick={() => navigate("/auth")} className="flex items-center gap-4 p-4 bg-white/5 border border-white/5 rounded-2xl cursor-pointer group hover:bg-white/[0.08] transition-all mb-8">
...
const MobileNav = () => {
    const location = useLocation();
    return (<div className="lg:hidden fixed bottom-0 left-0 right-0 h-20 bg-[#0A0907]/90 backdrop-blur-2xl border-t border-white/5 z-50 flex items-center justify-around px-6 pb-2">
      {menuItems.map((item) => {
            const isActive = location.pathname === item.href || (item.active && location.pathname === "/");
            return (<Link key={item.label} to={item.href} className={cn("flex flex-col items-center gap-1.5 transition-all relative", isActive ? "text-primary scale-110" : "text-muted-foreground opacity-50 hover:opacity-100")}>
...
export const Layout = ({ children }) => {
    return (<div className="flex min-h-screen bg-[#050505] text-white font-sans selection:bg-primary/30">
      <Sidebar />
