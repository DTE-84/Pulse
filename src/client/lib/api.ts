...
export const authAPI = {
  login: async ({ email, password }: any) => {
    ...
    if (!user) {
      const { data: newUser, error: insertError } = await supabase.from('dim_users').insert([{ 
        user_id_uuid: data.user.id, 
  ...
  signup: async ({ email, password, name }: any) => {
    ...
    if (data.user) {
      // Create profile in dim_users (linked to auth.users)
  ...
  me: async () => {
    ...
    if (!profile) {
      ...
      const { data: newProfile, error: insertError } = await supabase.from('dim_users').insert([{ 
    ...
    return { data: { ...profile, name: profile.user_name, id: profile.user_id_uuid, onboardingCompleted: profile.onboarding_completed } };
  },
...
export const transactionsAPI = {
  getAll: async (params?: any) => {
    ...
    let query = supabase.from('fact_transactions').select(`
      *,
  ...
  ingest: async (payload: any) => {
    ...
    const { data: result, error } = await supabase.from('fact_transactions').insert(formatted).select();
    if (error) throw error;
    return { data: result };
  }
};
...
export const goalsAPI = {
  ...
  create: async (data: any) => {
    ...
    const { data: result, error } = await supabase.from('dim_goals').insert([{
      user_id_uuid: user.id,
      goal_name: data.name,
...
export const statsAPI = {
  get: async () => {
    ...
    const { data: profile } = await supabase.from('dim_users').select('*').eq('user_id_uuid', user.id).single();
    const { data: txs } = await supabase.from('fact_transactions').select('*').eq('user_id_uuid', user.id);
    
    const totalSpent = txs?.reduce((acc, tx) => acc + parseFloat(tx.amount), 0) || 0;
    ...
    return {
      data: {
        ...
        chartData,
        novaInsight: "Your spending rhythm is stabilizing. Keep focus on the Emergency Vault.",
        triggers: [
...
export const novaServiceAPI = {
  chat: async (message: string, context?: any) => {
    ...
    try {
      ...
      console.warn("Nova: Falling back to Flash core due to signal deviation.", err);
      // Fallback: 1.5 Flash
      const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
      const result = await model.generateContent(`${message} (Low-latency mode active)`);
      const response = await result.response;
      return { data: { content: response.text() } };
    }
  },
  getInsights: async () => {
    return { data: { insights: ["Uplink stable.", "Data integrity nominal."] } };
  },
  getAnalysis: async () => {
    return { data: { analysis: "Behavioral telemetry indicates a balanced rhythm.", report: "Behavioral telemetry indicates a balanced rhythm.", monthlyExpenses: 0 } };
  }
};
