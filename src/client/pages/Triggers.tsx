...
const TriggerCard = ({ emoji, name, desc, severity, amount, count, peak, timeline, insight, novaSuggestion, colorClass }: any) => (
  <div className="bg-[#0A0907] border border-white/[0.03] rounded-[2.5rem] p-8 flex flex-col h-full group hover:border-white/10 transition-all hover:bg-[#0E0D0B] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] relative overflow-hidden">
    ...
    <div className="flex items-start gap-4 mb-8 relative z-10">
      <div className={cn('w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shrink-0 bg-white/[0.02] border border-white/5 shadow-2xl transition-all group-hover:scale-105', colorClass)}>
        {emoji}
      </div>
      <div className="flex-1 min-w-0">
        <h3 className="text-xl font-black text-white mb-1 tracking-tighter truncate uppercase">{name}</h3>
        <p className="text-[11px] text-muted-foreground font-bold uppercase tracking-widest leading-tight opacity-70">{desc}</p>
      </div>
    ...
    <div className="grid grid-cols-3 gap-4 mb-8 pt-6 border-t border-white/5">
      {[
        ...
        <div key={label}>
          <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1.5">{label}</div>
        ...
        {timeline.map((v: number, i: number) => (
          <div
            ...
            style={{
              ...
              backgroundColor: v > 0 ? (severity === 'HIGH' ? '#ef4444' : '#2DED9C') : 'rgba(255,255,255,0.05)',
    ...
    <div className="bg-primary/5 border border-primary/10 rounded-3xl p-5 flex items-center justify-between group/nova cursor-pointer hover:bg-primary/10 transition-all">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="text-[10px] font-bold text-primary uppercase tracking-[0.2em] mb-0.5">Nova Suggestion</p>
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <h1 className="text-4xl md:text-5xl font-black tracking-tighter text-white uppercase">Behavioral Triggers</h1>
            <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20 px-3 py-1 text-[10px] font-black uppercase tracking-widest animate-pulse">
              4 Patterns Active
            </Badge>
          </div>
          <p className="text-muted-foreground font-semibold text-sm max-w-2xl leading-snug">
            Deep analysis of behavioral spending nodes and impulse signals detected in your financial telemetry.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Trigger Spend" value="$684" trend="up" trendValue="$120" colorClass="bg-red-500" icon={Zap} />
        <StatCard label="Avg Pulse" value="$38" trend="down" trendValue="12%" colorClass="bg-yellow-500" icon={Activity} />
        <StatCard label="Interventions" value="12" trend="up" trendValue="+4" colorClass="bg-primary" icon={Brain} />
        <StatCard label="Risk Window" value="Thu PM" trend="down" trendValue="High" colorClass="bg-blue-400" icon={Clock} />
      </div>

      {/* Trigger cards grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <TriggerCard
          emoji="🍕" name="Stress Eating" severity="HIGH"
          desc="Work pressure → food delivery within 90 mins"
          amount="320" count="18" peak="Thu"
        ...
        />
        <TriggerCard
          emoji="🌙" name="Late Night Shop" severity="MED"
          desc="Impulse purchases after 10pm, mobile-driven"
          amount="186" count="9" peak="Fri"
        ...
        />
        <TriggerCard
          emoji="📱" name="Social Scroll" severity="LOW"
          desc="Browsing leads to checkout within 2 hrs"
          amount="98" count="4" peak="Sat"
        ...
        />
        <TriggerCard
          emoji="🥱" name="Boredom Spend" severity="MED"
          desc="Weekend time gap → browsing → buying"
          amount="80" count="5" peak="Sat"
      ...
      <div className="bg-[#0A0907] border border-white/[0.03] rounded-[3rem] p-8 md:p-12 relative overflow-hidden group hover:border-white/10 transition-all shadow-2xl">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10">
          ...
          <div className="flex items-center gap-5 text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground bg-white/5 px-6 py-3 rounded-full border border-white/5">
            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-sm bg-red-500/20" /> Low</div>
            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-sm bg-red-500/50" /> Elevated</div>
            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-sm bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]" /> Critical</div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto pb-4 scrollbar-hide">
          <div className="min-w-[600px] grid grid-cols-[80px_repeat(7,1fr)] gap-2 items-center">
            ...
            {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(d => (
              <div key={d} className="text-[10px] text-muted-foreground font-black text-center pb-3 uppercase tracking-widest">
                {d}
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-10">
        {/* Emotional Velocity */}
        <div className="bg-[#0A0907] border border-white/[0.03] rounded-[3rem] p-10 flex flex-col group hover:border-white/10 transition-all">
            ...
            {[
              ...
              <div key={e.n} className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[10px] font-black text-white">{e.p}%</span>
                </div>
          ...
          <div className="mt-auto pt-8 border-t border-white/[0.05]">
            <p className="text-xs text-muted-foreground leading-relaxed italic">
              <span className="text-primary font-black not-italic uppercase tracking-widest mr-2">Nova:</span> 
              "Stress is your primary driver — addressing work stress could reduce impulse volume by 40%."
            </p>
          </div>
        </div>
        ...
        <div className="bg-[#0A0907] border border-white/[0.03] rounded-[3rem] p-10 flex flex-col group hover:border-white/10 transition-all">
          ...
          <div className="space-y-4">
            {[
              { icon: '🧘', label: 'Breathing Protocol', sub: 'Reduced impulse buys by 34%',      eff: '+34%',  status: 'good' },
              { icon: '⏱️', label: '24-hr Wishlist Rule', sub: 'Saved $312 in cycle delta',        eff: '$312',  status: 'good' },
              { icon: '🚶', label: 'Physical Reset',      sub: 'Nova suggested · Higher efficiency', eff: 'New', status: 'new'  },
              { icon: '💬', label: 'Nova Direct Sync',   sub: 'Reduces spend urge by 60%',         eff: '+60%',  status: 'good' },
            ].map(r => (
              <div key={r.label}
                ...
                <span className="text-2xl group-hover/item:scale-110 transition-transform">{r.icon}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-black text-white tracking-tight uppercase">{r.label}</div>
                  <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider truncate">{r.sub}</div>
                </div>
                <div className={cn(
